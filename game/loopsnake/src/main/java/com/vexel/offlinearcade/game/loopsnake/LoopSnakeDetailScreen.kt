...
fun LoopSnakeDetailScreen(
    ...
    EdgeToEdgeAppScaffold(
        ...
        HeroPanel(
            ...
            title = "Loop Snake",
            subtitle = "Collect orbs, grow your snake, and avoid collisions.",
        )

        ArcadeCard {
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(spacing.sm)) {
                PremiumStatTile(label = "Best Score", value = (stats?.highScore ?: 0).toString(), modifier = Modifier.weight(1f), accent = ArcadeTheme.colors.loopSnakeAccent)
        ...
        ArcadeCard {
            ...
            Column(verticalArrangement = Arrangement.spacedBy(spacing.sm)) {
                Text("• Swipe to change the snake's direction.", color = ArcadeTheme.colors.textSecondary)
                Text("• Collect orbs to grow longer and score points.", color = ArcadeTheme.colors.textSecondary)
                Text("• Avoid hitting the walls or your own tail.", color = ArcadeTheme.colors.textSecondary)
                Text("• The game ends when you collide with an obstacle.", color = ArcadeTheme.colors.textSecondary)
            }
        }
